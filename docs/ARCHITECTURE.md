...
| **Measurement Repository** | Owns authoritative persistence and retrieval of SpectrumMeasurements and observation/sweep coverage records. Enforces retention and bounded intake behavior. | Valid measurements, observation/sweep outcomes, retention policy → durable source records, retrieval/query results, pressure state. | Measurement/coverage records, ingestion outcome, retention bookkeeping; local prototype uses persistent SQLite. | Receives only normalized data; serves Baseline/Detection and Analytics; publishes storage health. | Temporary persistence fault: record a storage incident and fail the affected write; acquisition/recovery policy surfaces the condition. Capacity pressure and production retention policy remain deployment concerns. |
...
| **Heatmap and Report Generator** | Creates `report.json`, `report.txt`, `waterfall.png`, and `heatmap.png` from SQLite-backed report windows, records coverage, and makes artifacts retrievable. | SQLite report query, calendar schedule or authorized last-hour request → report package and generation status. | Derived artifact metadata and generation job state; not authoritative source facts. | Invoked by report scheduler/Telegram handler; artifacts delivered through Telegram. | Rendering/generation failure: retry or report failure; never halt acquisition or alter source records. |
...
The local prototype selects SQLite for canonical sweep storage and incident history at
`runtime/sweeps.sqlite3`; this does not settle all future domain stores or production
retention thresholds. The architecture requires persistent categories with distinct access/retention behavior:
...
| Measurement/Event Repositories | SQLite in the local prototype; production persistence and retention validation remain open | Isolated fixture repository with stored datasets and injected errors | Detector, event history, retention, report and failure behavior. |
...
The architecture intentionally does not settle adapter/tool adequacy beyond the measured
local RTL-SDR baseline, SoapySDR module behavior, stable device identity, concurrency,
baseline method, production retention values, IQ policy, or classifier feasibility.
The local prototype fixes `Europe/Kyiv` as the default timezone, hourly-at-hour-start and
daily-at-midnight report boundaries, and an authorized last-hour Telegram query; remaining
uncertainties are maintained in [ARCHITECTURE_QUESTIONS.md](ARCHITECTURE_QUESTIONS.md).
...
сотні послідовних tuner hops; practical full-range baseline — 24–1766 МГц із
operational cadence budget 60 с. Acquisition не імпортує й не чекає reporting/Telegram;
report generation відбувається окремим consumer. Деталі та компроміси: [Continuous acquisition](CONTINUOUS_ACQUISITION.md).
