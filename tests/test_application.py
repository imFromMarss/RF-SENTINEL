...
def test_survey_mode_composes_without_hardware(monkeypatch, capsys, tmp_path):
    ...
    assert "Survey: success" in capsys.readouterr().out


def test_schedule_mode_uses_configured_interval(monkeypatch):
    monkeypatch.setenv("RF_SENTINEL_TELEGRAM_BOT_TOKEN", "")
    monkeypatch.setenv("RF_SENTINEL_TELEGRAM_CHAT_ID", "")
    monkeypatch.setenv("RF_SENTINEL_REPORT_INTERVAL_MINUTES", "7")
    calls = []
    monkeypatch.setattr("rf_sentinel.scheduler.run_schedule",
                        lambda run, interval, stop: calls.append((run, interval, stop)))
...
def test_invalid_configuration_has_safe_output(monkeypatch, capsys):
    ...
    assert "configuration or survey failed" in output.err
...
def test_scheduler_keyboard_interrupt_exit(monkeypatch):
    ...
    monkeypatch.setattr("rf_sentinel.scheduler.run_schedule", interrupt)
